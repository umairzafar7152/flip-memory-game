...
import kotlinx.android.synthetic.main.fragment_single_player74.*
import com.chumairzafar.flipmemory.R.drawable.*

class SinglePlayerFragment74 : Fragment() {

    private var lastCard = -1
    private var attempts = 24
    private var found = 0
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_single_player74, container, false)
    }
...
//                                imageViews[lastCard].setImageResource(cardBack)
                                imageViews[lastCard].contentDescription = "cardBack"

                                passData(attempts, false)
                                //  enable whole activity touch
                                activity!!.window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE)
                            }
                            lastCard = -1
                            enableTouch(imagesLayout)
                            if (attempts <= 0 && found != 14) {
                                notifyGameOver()
                            }
                        }, 600)
                    }
